...
void p2p_ssd_to_host(int &nvmeFd, xrtDeviceHandle device, xrt::kernel &krnl) {
  ...
  std::cout << "Map Global Memory Buffer to Host Pointer" << std::endl;
  // Map the contents of the buffer object into host memory
  auto p2p_bo0_map = p2p_bo0.map<int *>();
  auto bo_out_map = bo_out.map<int *>();
  std::cout << "Fill the input buffer with data" << std::endl;
...
int main(int argc, char **argv) {
  ...
  if ((xcl_mode != nullptr) && !strcmp(xcl_mode, "hw_emu")) {
    if (device_name.find("2018") != std::string::npos) {
      std::cout << "[INFO]: This code is not supported for " << device_name
  ...
  if (filepath.empty()) {
    std::cout << "\nWARNING: As file path is not provided using -p option, "
                 "going with -f option which is local "
                 "file testing. Please use -p option, if looking for actual "
                 "p2p operation on NVMe drive.\n";
    filename = parser.value("input_file");
  } else {
    std::cout << "\nWARNING: Ignoring -f option when -p options is set. -p has "
                 "high precedence over -f.\n";
    filename = filepath;
  }
